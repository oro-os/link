...
impl<'a, const N: usize> Client<'a, N> {
	pub fn new(sock: TcpSocket<'a>, buf: &'a mut heapless::Vec<u8, N>) -> Self {
		Self { sock, buf }
	}

	pub async fn ping(&mut self) -> Result<()> {
		let rn = crate::rand::next_u32();
		RedisWriter::start(self.buf, "PING", 1)?.arg(rn)?.finish(&mut self.sock).await?;

		let expected = heapless::format!(64; "${}\r\n{}\r\n", WriteCounter::new(&rn), rn).unwrap();
		self.buf.resize(expected.len(), 0).map_err(|_| Error::TooLong)?;
		self.sock.read_exact(&mut self.buf[..expected.len()]).await?;
	...
	pub async fn set(&mut self, key: impl Display, value: impl Display) -> Result<()> {
		RedisWriter::start(self.buf, "SET", 2)?.arg(key)?.arg(value)?.finish(&mut self.sock).await?;
		let expected = b"+OK\r\n";
		self.buf.resize(expected.len(), 0).map_err(|_| Error::TooLong)?;
		self.sock.read_exact(&mut self.buf[..expected.len()]).await?;
	...
	pub async fn get<R: core::str::FromStr>(&mut self, key: impl Display) -> Result<Option<R>> {
		RedisWriter::start(self.buf, "GET", 1)?.arg(key)?.finish(&mut self.sock).await?;
		...
		match self.buf[0] {
			b'$' => {
				...
				let s = core::str::from_utf8(&self.buf[..len]).map_err(|_| Error::UnexpectedResponse)?;
	...
	async fn read_length(&mut self) -> Result<Option<usize>> {
		...
		let len_str = core::str::from_utf8(&len_buf[..pos]).map_err(|_| Error::UnexpectedResponse)?;
		let len = len_str.parse::<usize>().map_err(|_| Error::UnexpectedResponse)?;
		Ok(Some(len))
	}
}
...
impl<'a, const N: usize> RedisWriter<'a, N> {
	fn start(buf: &'a mut heapless::Vec<u8, N>, cmd: &'static str, nargs: usize) -> Result<Self> {
		buf.clear();
		core::write!(buf, "*{}\r\n${}\r\n{}\r\n", 1 + nargs, cmd.len(), cmd).map_err(|_| Error::TooLong)?;
		Ok(Self { buf })
	}

	fn arg(mut self, arg: impl Display) -> Result<Self> {
		core::write!(self.buf, "${}\r\n{}\r\n", WriteCounter::new(&arg), arg).map_err(|_| Error::TooLong)?;
		Ok(self)
	}
