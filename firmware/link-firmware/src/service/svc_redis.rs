...
pub async fn run(rx: &'static Channel, config: Config) -> ! {
	...
	defmt::info!("connecting to redis (via area controller) at {:?})...", endpoint);
	sock.connect(endpoint).await.expect("failed to connect to redis");
	defmt::info!("connected to redis at {:?}; pinging...", endpoint);

	let mut client = crate::redis::Client::new(sock, cmd_buf);
	...
	loop {
		...
		client.set("debug:counter", format_args!("count={counter}")).await.expect("failed to set COUNTER in redis");
		let message: Option<heapless::String<64>> = client.get("debug:message").await.expect("failed to get MESSAGE from redis");
		if let Some(message) = message {
			defmt::info!("got message from redis: {}", message);
		} else {
			defmt::debug!("no message in redis");
		}
		let count: Option<u32> = client.get("debug:server-count").await.expect("failed to get COUNT from redis");
